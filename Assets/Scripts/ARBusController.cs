...
/* 
    ...
    public void advanceScene(){
        ...
        switch (busCounter){
            ...
            case 0:
                ...
                break;
            case 1:
                instuctionText.text = "Wait for the bus to stop";
                Bus.SetActive(true);
                int i= 0;
                while(i <= 40){
                    instuctionText.text = "Wait for the bus to stop" + i;
                    Bus.transform.position += new Vector3(0,1,0) * Time.deltaTime * 1;
                    i++;
                }
                busCounter++;
                Debug.Log("Test2");
            ...
            case 2:
                int j= 0;
                while(j <= 10){
                    Bus.transform.position += new Vector3(-1,0,0) * Time.deltaTime * 1;
                    instuctionText.text = "Then you get on the bus"  + j;;
                    j++;
                }
                busCounter++;
                break;
            case 3:
                if(inZone == true){
                    instuctionText.text = "Next pay for your ride at the counter";
                }
                busCounter++;
                break;
            case 4:
                instuctionText.text = "Then find an open seat";
                busCounter++;
                break;
            case 5:
    ...
    public void placeObject(){
        if (spawned != true){
            GameObject obj = Instantiate(BusController, placementIndicator.transform.position, placementIndicator.transform.rotation);
            spawned = true;
        }
    }
    /* public void placeObject(){
